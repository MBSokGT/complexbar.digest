...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Индикатор загрузки - удаляем немедленно
  ...
  if (fortuneSection) {
    const fortunePhrases = [
      'Твоё упорство принесёт долгожданный успех!',
      'Важно отпустить старое, чтобы впустить новое.',
      'Ты найдёшь время на то, что давно хотелось попробовать.',
      'Твои маленькие шаги приведут к большим радостям.',
      'Новая встреча принесёт неожиданно хорошие эмоции.',
      'Кто-то удивит тебя приятной мелочью.',
      'Маленькая радость придёт совсем неожиданно.',
      'Не забывай устраивать мини-побег из рутины.',
      'Новый год — повод сделать то, что давно откладывалось.',
      'В новом году можно позволить себе маленькую радость.',
      'В новом году всё будет чуть проще, чем кажется.',
      'Новый год начнётся с улыбки.',
    ...
    const fortuneHTML = `
      ...
      <div class="fortune-cookie-container" style="background: #fff; border-radius: 20px; padding: 40px; margin-bottom: 32px; box-shadow: 0 8px 32px rgba(0,0,0,0.15); text-align: center;">
        <div class="fortune-screen1">
          <h3 style="color: #be0318; font-size: 2rem; margin-bottom: 16px; font-weight: 700;">🥠 Выбери печеньку, которая скажет тебе, что ждать в новом году!</h3>
        ...
        <div class="fortune-screen2" style="display: none;">
          ...
          <img src="assets/cookie2.svg" alt="Открытое печенье" style="width: 180px; height: auto;" />
        </div>
      </div>
    `;

    const h2 = fortuneSection.querySelector('h2');
    if (h2) {
      h2.insertAdjacentHTML('afterend', fortuneHTML);
    } else {
  ...
  // Бинго
  const bingoTasks = ['Нарезать ингредиенты для салатов ножами Arcos','Сделать глинтвейн с новинкой от Pinch&Drop','Пить шампанское из бокалов Arc Reveal\'Up','Отметить Новый год по всем часовым поясам','Загадать заветное желание под бой курантов','Потанцевать под любимый трек года','Спеть в караоке в «Евгениче»','Посмотреть сериал <a href="https://www.kinopoisk.ru/series/4481731/" target="_blank" style="color:#be0318;text-decoration:none;display:inline;">«Медведь»</a>','Покататься на коньках','Прогуляться по пустым улицам утром 1 января','Позавтракать в «Горыныче»','Сформулировать одно желание на весь 2026 год'];
  ...
  if (bingoSection) {
    ...
    bingoSection.appendChild(bingoCard);
    ...
    // Добавляем счетчик после бинго
    const countdownHTML = `
      <div id="countdown" role="timer" aria-live="polite" aria-label="Обратный отсчет до Нового Года" style="background: rgba(255,255,255,0.95); border-radius: 16px; padding: 24px; margin: 32px auto; max-width: 600px; box-shadow: 0 8px 32px rgba(0,0,0,0.15);">
        <div style="text-align: center; color: #2a0808; margin-bottom: 16px; font-size: 1.1rem; font-weight: 600;">До Нового Года осталось:</div>
        <div style="display: flex; justify-content: center; gap: 16px; flex-wrap: wrap;">
          <div style="text-align: center;">
            <div id="days" style="font-size: 2.5rem; font-weight: 900; color: #be0318; line-height: 1;">0</div>
            <div style="font-size: 0.85rem; color: #4c0010; margin-top: 4px;">дней</div>
          </div>
          <div style="text-align: center;">
            <div id="hours" style="font-size: 2.5rem; font-weight: 900; color: #be0318; line-height: 1;">0</div>
            <div style="font-size: 0.85rem; color: #4c0010; margin-top: 4px;">часов</div>
          </div>
          <div style="text-align: center;">
            <div id="minutes" style="font-size: 2.5rem; font-weight: 900; color: #be0318; line-height: 1;">0</div>
            <div style="font-size: 0.85rem; color: #4c0010; margin-top: 4px;">минут</div>
          </div>
          <div style="text-align: center;">
            <div id="seconds" style="font-size: 2.5rem; font-weight: 900; color: #be0318; line-height: 1;">0</div>
            <div style="font-size: 0.85rem; color: #4c0010; margin-top: 4px;">секунд</div>
          </div>
        </div>
      </div>
    `;
    bingoSection.insertAdjacentHTML('beforeend', countdownHTML);
    ...
    function updateCountdown() {
      ...
      if (diff <= 0) {
        countdownEl.innerHTML = '<div style="text-align: center; color: #be0318; font-size: 2rem; font-weight: 900;">🎉 С Новым Годом! 🎉</div>';
        return;
      }
